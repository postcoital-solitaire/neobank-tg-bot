...
class BaseModel(AbstractModel):
    ...
    async def get_many(self,
        ...
        return [self.__class__(**dict(fields)) for fields in result]
    ...
    async def update(self,
                     cur: aiomysql.Cursor,
                     where_field: str = 'docx_first_page'):
        answer = await cur.execute(
            f"UPDATE `{self.TABLE_NAME}` SET {', '.join(f'`{k}`=%s' for k in self.fields.keys())} "
            f"WHERE `{where_field}` = %s;",
            tuple(self.fields.values()) + (self.fields.get(where_field),))
