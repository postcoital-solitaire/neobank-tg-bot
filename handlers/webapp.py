...
@router.message(TypicalFilter(for_replace=["/setmenubutton", "вебапп", "/web"]))
async def webapp_start(message: Message, state: FSMContext):
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Авторизация", web_app=WebAppInfo(url="https://lively-cranachan-dffc27.netlify.app/"))]
        ]
    )
